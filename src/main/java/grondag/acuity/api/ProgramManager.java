...
public final class ProgramManager implements IProgramManager
{
    ...
    private ProgramManager()
    {
        ...
        this.waterProgram = createProgram(
                PipelineShaderManager.INSTANCE.getOrCreateVertexShader("/assets/acuity/shader/water.vert", TextureFormat.SINGLE),
                PipelineShaderManager.INSTANCE.getOrCreateFragmentShader("/assets/acuity/shader/water.frag", TextureFormat.SINGLE))
                .finish();
        
        this.lavaProgram = createProgram(
                PipelineShaderManager.INSTANCE.getOrCreateVertexShader("/assets/acuity/shader/lava.vert", TextureFormat.SINGLE),
                PipelineShaderManager.INSTANCE.getOrCreateFragmentShader("/assets/acuity/shader/lava.frag", TextureFormat.SINGLE))
                .finish();
    }
    ...
    synchronized public IProgram createProgram(IPipelineVertexShader vertexShader, IPipelineFragmentShader fragmentShader)
    {
        ...
        {
            Acuity.INSTANCE.getLog().error(I18n.translateToLocalFormatted("misc.error_mismatched_vertex_formats", 
                    ((PipelineVertexShader)vertexShader).fileName, ((PipelineFragmentShader)fragmentShader).fileName));
        }
